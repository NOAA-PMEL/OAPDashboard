        ...
        try  {
            ...
            if ( spacer == null ) {
                ...
                if ( spacer == null || delimiter != spacer.charValue() ) {
                    if ( spacer != null ) {
                        logger.warn("Parser detected different delimiter '" + delimiter + "' from specified value '" + spacer + "'." );
                    }
                    spacer = delimiter;
                }
            }
                    ...
                    if ( StringUtils.emptyOrNull(specifiedDatasetId)) {
        				...
        				if ( (datasetId.length() < DashboardServerUtils.MIN_DATASET_ID_LENGTH) ||
        					...
        					throw new IllegalStateException("Invalid dataset ID \"" + datasetId + "\" from dataset name \"" 
    										   + datasetName + "\" at record number: " + dataParser.getContext().currentLine()
    										   +". DatasetID must be between " + DashboardServerUtils.MIN_DATASET_ID_LENGTH
