 ...
 *
 */
public class ProfileUploadProcessor extends FileUploadProcessor {

    public ProfileUploadProcessor(StandardUploadFields uploadFields) {
        super(uploadFields);
    }
    ...
    public void doFeatureSpecificProcessing(List<FileItem> datafiles) throws UploadProcessingException {
		...
		for ( FileItem item : datafiles ) {
			...
			for ( DashboardDatasetData datasetData : datasetsMap.values() ) {
                datasetData.setFeatureType(FeatureType.PROFILE.name());
